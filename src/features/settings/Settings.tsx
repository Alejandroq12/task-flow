...
import type { ProfileQuery } from '@/graphql/generated/graphql'
...
function ProfileCard({ user }: { user: ProfileQuery['profile'] }) {
  return (
    <section
      aria-label="Profile"
    ...
    >
      <div className="flex items-center gap-4">
        <img className="size-16 rounded-full" src={avatarSrc(user.avatar) ?? avatarUrl} alt="" />
        <div className="min-w-0">
          <h2 className="truncate text-body-xl font-semibold text-neutral-1">{user.fullName}</h2>
          <p className="truncate text-body-m text-neutral-2">{user.email}</p>
        </div>
      </div>
      <dl className="grid gap-x-8 gap-y-4 sm:grid-cols-2">
        <div className="flex flex-col gap-1">
          <dt className="text-body-s text-neutral-2">Full name</dt>
          <dd className="text-body-m text-neutral-1">{user.fullName}</dd>
        </div>
        <div className="flex flex-col gap-1">
          <dt className="text-body-s text-neutral-2">Email</dt>
          <dd className="text-body-m text-neutral-1">{user.email}</dd>
        </div>
        <div className="flex flex-col gap-1">
        ...
        <div className="flex flex-col gap-1">
          ...
          <dd className="text-body-m text-neutral-1">{formatDate(user.createdAt)}</dd>
        </div>
        <div className="flex flex-col gap-1">
          <dt className="text-body-s text-neutral-2">Updated at</dt>
          <dd className="text-body-m text-neutral-1">{formatDate(user.updatedAt)}</dd>
        </div>
      </dl>
    </section>
  )
...
export function Settings() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {profile.data !== undefined && <ProfileCard user={profile.data.profile} />}
    </div>
  )
}
